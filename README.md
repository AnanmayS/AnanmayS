hey, i'm ananmay 👋

computer engineering @ umd (class of 2028)

i build things with python, typescript, and whatever else gets the job done.
into ai, financial systems, and automation that actually works.

### what i'm working on

- **showdown** — reinforcement learning trading agent
- **polymarket paper trader** — prediction market simulation engine
- **friday** — personal ai automation assistant

### tech i use

python · typescript · react · fastapi · docker · aws · postgres · supabase

### find me

[linkedin](https://linkedin.com/in/ananmaysingh) · [website](https://ananmaysingh.dev)
